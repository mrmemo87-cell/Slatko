...
export const VisitPage: React.FC = () => {
  ...
  // Initialize delivery items from prepared order
  useEffect(() => {
    if (orders && clientId && deliveryItems.length === 0) {
      const preparedOrder = orders.find(o => o.clientId === clientId && o.status === 'PREPARED');
      if (preparedOrder && preparedOrder.items) {
        setDeliveryItems(preparedOrder.items.map(i => ({ id: i.productId, qty: i.quantity })));
      }
    }
  }, [orders, clientId]);

  if (clientsLoading || productsLoading || ordersLoading) {
    return <div className="p-8 text-center text-slate-500">Loading visit details...</div>;
  }
  ...
  const TabButton = ({ id, label, icon }: { id: typeof activeTab, label: string, icon: React.ReactNode }) => (
    <button
      ...
      className={`flex-1 py-3 flex flex-col items-center gap-1 border-b-2 transition-all ${activeTab === id
              ...
              return (
                <div key={item.id} className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100">
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <span className="font-bold text-red-500">-${((product?.price || 0) * item.qty).toFixed(2)}</span>
                  </div>
              ...
              return (
                <div key={item.id} className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100">
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <span className="font-bold text-slate-800">${((product?.price || 0) * item.qty).toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between items-center bg-slate-50 rounded-xl p-2">
                    <span className="text-xs font-bold text-slate-400 uppercase ml-2">Quantity</span>
            ...
            <div className="grid grid-cols-3 gap-2">
              {['CASH', 'BANK_TRANSFER', 'CHECK'].map(m => (
                <button
                  ...
                  className={`py-3 px-1 text-[10px] md:text-xs font-bold uppercase rounded-xl border-2 transition-all ${paymentMethod === m
