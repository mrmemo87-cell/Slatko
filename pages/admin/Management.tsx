...
import { MOCK_CLIENTS, MOCK_PRODUCTS, MOCK_USERS, MOCK_PURCHASES } from '../../constants';
...
export const ManagementPage: React.FC<ManagementPageProps> = ({ type }) => {
  ...
  const config = {
    ...
    products: {
      ...
      data: MOCK_PRODUCTS.filter(p => p.category !== 'Raw Material'),
      columns: [
        ...
        { header: 'Price', accessor: (item: any) => `$${item.price.toFixed(2)}` },
    ...
    inventory: {
      ...
      columns: [
        ...
        { header: 'Value', accessor: (item: any) => `$${(item.price * item.stock).toFixed(2)}` },
      ]
    },
    users: {
      title: 'System Users',
      data: MOCK_USERS,
      columns: [
        { header: 'User', accessor: (item: any) => <div className="flex items-center gap-2"><div className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center font-bold text-xs">{item.name[0]}</div><span className="font-bold">{item.name}</span></div> },
    ...
    purchases: {
        ...
        columns: [
            ...
            { header: 'Total', accessor: (item: any) => <span className="font-bold">${item.total.toFixed(2)}</span> },
